...
export default function Reports() {
  ...
  const getStudentRank = (studentId, studentClass) => {
    ...
    // Ranking based on Total Score of the current term
  ...
  async function handleSaveDetails() {
    ...
    // Helper to get value from edits or original, fallback to 0
    const getVal = (key) => Number((s[key] !== undefined ? s[key] : original[key]) || 0);

    // Calculate Total Score based on ALL 6 Subjects
    const newTotalScore = 
  ...
  const getValue = (studentId, baseField) => {
      // Use getField which automatically adds t1_ or t2_ prefix based on currentTerm
  ...
  const downloadTemplate = () => {
    ...
    const rows = studentsToExport.map(s => {
      ...
      // Add tab (\t) to ID to prevent Excel treating it as formula
      const safeId = `"\t${s.id}"`; 
  ...
  const handleFileUpload = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (user.role === 'teacher' && student.class !== user.class) { failed++; continue; }

        const term = termRaw ? Number(termRaw) : currentTerm;
        const prefix = `t${term}_`; // Ensures t1_ or t2_
        ...
        // Update Total Score with the new sum
        updates[`${prefix}total_score`] = currentTotal;

        // Attendance & Cocu are now Term Specific in this new data structure
        // Using _attendance_days and _cocu_days keys
        if (att !== "" && !isNaN(Number(att))) updates[`${prefix}attendance_days`] = Number(att);
        if (cocu !== "" && !isNaN(Number(cocu))) updates[`${prefix}cocu_days`] = Number(cocu);
    ...
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12">
       ...
       <div className="bg-white rounded-2xl p-5 shadow-xl shadow-slate-200/50 mb-8 border border-slate-100 flex flex-col md:flex-row gap-4 items-center relative z-10">
        ...
        {/* NEW: Import/Export Buttons for Teacher/Admin */}
      ...
      <div className="overflow-x-auto pb-10">
        <table className="w-full text-left border-separate border-spacing-y-4">
            ...
            {viewList.length > 0 ? (
                viewList.map(s => {
                // ATTENDANCE & COCU Calculation using Term Specific Fields
